...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------*/	
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			if( selection instanceof ITextSelection && part instanceof PictureStreamEditorPart){
							
				ITextSelection text = (ITextSelection)selection;
				
				if( text.getText() != null && !text.isEmpty() ){
									
					PictureStreamEditorPart texteditor = (PictureStreamEditorPart)part;
					
					//add and send the test sequence
					
						((PictureModel)texteditor.getModel()).addTestText( text.getText() );
						((PictureModel)texteditor.getModel()).sendTestSequence();
					...
					//create operation for (undo/redo)
						//AddTextVerificationOperation op = new AddTextVerificationOperation( (PictureModel)texteditor.getModel(), text.getText() );
