...
public class FirefoxChromeLauncher extends AbstractBrowserLauncher {
    ...
    public FirefoxChromeLauncher(SeleniumConfiguration seleniumConfiguration) {
        this(seleniumConfiguration, findBrowserLaunchLocation());
    }

    public FirefoxChromeLauncher(SeleniumConfiguration seleniumConfiguration, String browserLaunchLocation) {
    ...
    protected void launch(String url) {
        try {        
            String homePage = new ChromeUrlConvert().convert(url, getSeleniumConfiguration().getPort());
    ...
    private String makeCustomProfile(String homePage) throws IOException {
        ...
        String firefoxProfileTemplate = getSeleniumConfiguration().getFirefoxProfileTemplate();
        if (firefoxProfileTemplate != null && !"".equals(firefoxProfileTemplate)) {
            File firefoxProfileTemplateFile = new File(firefoxProfileTemplate); 
            LauncherUtils.copyDirectory(firefoxProfileTemplateFile, customProfileDir);
        }
        ResourceExtractor.extractResourcePath(getClass(), sourceLocationName, customProfileDir);

        copyRunnerHtmlFiles();

        LauncherUtils.generatePacAndPrefJs(customProfileDir, getSeleniumConfiguration().getPort(), LauncherUtils.ProxySetting.NO_PROXY, homePage, changeMaxConnections);
    ...
    private void copyRunnerHtmlFiles() {
        ...
        LauncherUtils.extractHTAFile(htmlDir, getSeleniumConfiguration().getPort(), "/core/TestRunner.html", "TestRunner.html");
        LauncherUtils.extractHTAFile(htmlDir, getSeleniumConfiguration().getPort(), "/core/RemoteRunner.html", "RemoteRunner.html");
