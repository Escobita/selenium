...
public class WindowsProxyManager {
    ...
    public WindowsProxyManager(boolean customPACappropriate, Session session, int port) {
        this.customPACappropriate = customPACappropriate;
        this.port = port;
        customProxyPACDir = LauncherUtils.createCustomProfileDir(session);
    ...
    protected void changeRegistrySettings(SeleniumConfiguration seleniumConfiguration) throws IOException {
    	...
    	if (!customPACappropriate) {
    		String proxyServer = "127.0.0.1:" + seleniumConfiguration.getPort();
            settings = new HudsuckrSettings(oldSettings.connection, true, true, false, false, proxyServer, "(null)", "(null)");
        } else {
            File proxyPAC = LauncherUtils.makeProxyPAC(customProxyPACDir, seleniumConfiguration, seleniumConfiguration.isAlwaysProxy());
...
//        }
        ...
        if (seleniumConfiguration.isEnsureCleanSession()) {
    ...
    public void restoreRegistrySettings(SeleniumConfiguration seleniumConfiguration) {
    ...
    private String runHudsuckr(String... args) {
    	...
    	// @todo Add AntJettyLoggerBuildListener
