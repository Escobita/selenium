...
function SocketListener(server, transport) {
  ...
  this.wm = Utils.getService("@mozilla.org/appshell/window-mediator;1",
      "nsIWindowMediator");
  this.server = server;
}
...
SocketListener.prototype.executeCommand = function() {
  ...
  var respond = {
    send : function() {
      ...
      var converter = Utils.newInstance(
  ...
  if (command.commandName == "findActiveDriver" ||
    ...
    var webNav = activeWindow.QueryInterface(CI.nsIInterfaceRequestor).
        getInterface(CI.nsIWebNavigation);
    var loadGroup = webNav.QueryInterface(CI.nsIInterfaceRequestor).
      ...
      if (info.webProgress.isPending()) {
        ...
        if (info.driver.window.location == "about:blank" && !info.onBlank) {
          ...
          try {
            ...
            info.driver[info.command.commandName](respond,
                info.command.parameters);
          } catch (e) {
            if (e instanceof StaleElementError) {
              ...
              var message = "Exception caught by driver: " +
...
SocketListener.prototype.switchToWindow = function(response, windowId,
                                                   opt_isSecondSearch) {
...
SocketListener.prototype.quit = function() {
  var appService =
      Utils.getService("@mozilla.org/toolkit/app-startup;1", "nsIAppStartup");
