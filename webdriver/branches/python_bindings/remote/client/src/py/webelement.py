...
class WebElement(object):
    ...
    def find_element_by_id(self, id):
        """Finds element by id."""
        return self._get_elem(self._post("element/id", {"using": "id", "value": id})[0])

    def find_element_by_name(self, name):
        """Find element by name."""
        return self._get_elem(self._post("element/name", {"using": "name", "value": name})[0])

    def find_element_by_link_text(self, link_text):
        """Finds element by link text."""
        return self._get_elem(self._post("element/link%20text", {"using": "link text", "value": link_text})[0])
    ...
    def find_elements_by_xpath(self, xpath):
        ...
        resp = self._post("elements/xpath", {"using": "xpath", "value": xpath})
        elems = []
        for v in resp:
