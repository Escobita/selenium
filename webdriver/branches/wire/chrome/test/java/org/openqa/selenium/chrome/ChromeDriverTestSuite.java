    ...
    private static ChromeExtension createExtension() {
      ...
      try {
        ...
        for (File file : extensionSrcDir.listFiles()) {
          FileHandler.copy(file, new File(extensionDstDir, file.getName()));
        }

        if (!Platform.getCurrent().is(Platform.WINDOWS)) {
          File dllFrom =
              new File(extensionSrcDir, "../../../build/Win32/Release/npchromedriver.dll");
          if (!dllFrom.exists()) {
            throw new FileNotFoundException("Could not find " + dllFrom.getCanonicalFile());
          }
          FileHandler.copy(dllFrom, new File(extensionDstDir, dllFrom.getName()));
        }
      } catch (IOException e) {
