  ...
  for (id element in arguments) {
    if ([element isKindOfClass:[NSArray class]]) {
      ...
      if ([type isEqualToString:@"ELEMENT"]) {
        NSString *idString = (NSString *) value;
        [converted appendString:[elementStore_ jsLocatorForElementWithId:idString]];
      } else {
...
- (NSDictionary *)executeScript:(NSDictionary *)arguments {
  ...
  NSArray *args = [arguments objectForKey:@"args"];
  NSString *argsAsString = [self convertArgumentsToJavascript:args];
  ...
  NSString *result = [[self viewController] jsEval:
                      @"var result, error = null;\r"
                      "(function() {\r"
                      "  var f = function(){%@};\r"
                      "  try {\r"
                      "    result = f.apply(null, %@);\r"
                      "  } catch (e) {\r"
                      "    error = e.message || e.toString();\r"
                      "  }\r"
                      "  return result;\r"
                      "})();", code, argsAsString];
  ...
  // back to the client.  Note that arrays and objects are not supported.
  NSString *type = [[self viewController] jsEval:
                    @"(function() {\r"
                    "  if (null === result || undefined === result) {\r"
                    "    return 'NULL';\r"
                    "  } else if (result instanceof HTMLElement) {\r"
                    "    return 'ELEMENT';\r"
                    "  } else {\r"
                    "    return (typeof result).toUpperCase();\r"
                    "  }\r"
                    "})();"];
  ...
  if ([type isEqualToString:@"NULL"]) {
    ...
    if ([type isEqualToString:@"NUMBER"]) {
      ...
      // Result could be a string, array, or generic object.  Currently,
      // we do not support arrays or generic objects (which could conceivably be
      // converted to a map).
